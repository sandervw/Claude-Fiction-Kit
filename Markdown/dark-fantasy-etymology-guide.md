# Dark Fantasy Etymology Guide

A ruleset for generating names and terms consistent with the established linguistic aesthetic.

---

## Core Phonological Rules

### 1. Consonant Clusters (Heavily Favored)

Initial clusters give names weight and texture. Use liberally:

| Cluster  | Examples                                      |
| -------- | --------------------------------------------- |
| **Thr-** | Throssvel, Thremnir, Thelvash, Thrundle       |
| **Skr-** | Skrelving, Skrivvi, Skrella                   |
| **Sk-**  | Skelvorn, Skolla, Skurr                       |
| **Dr-**  | Drhum, Drenn, Droz, Drossam                   |
| **Gr-**  | Grothven, Grundvik, Grelb                     |
| **Br-**  | Bremm, Branvik, Brollix                       |
| **Kr-**  | Kremlik, Krossam, Krannogg                    |
| **Ph-**  | Phalantios, Phodric, Phoss (use "ph" not "f") |
| **Kl-**  | Klokhan, Kleyt, Clodrik                       |
| **St-**  | Strath, Stobb, Stober                         |
| **Vr-**  | Vrinn, Vross, Vriss                           |
| **Pl-**  | Plax, Plossk, Plixie                          |

### 2. Geminate (Doubled) Consonants

Very common—creates a "thick" sound:

- **-ss**: Phoss, Vross, Dross, Kess
- **-nn**: Vrenn, Drenn, Glenn, Benn
- **-ll**: Threll, Skolla, Thrall
- **-bb**: Globb, Throbb, Clobba
- **-tt**: Tharbett, bivallat
- **-mm**: Bremm, Dremm, Gremmi

### 3. Aspirated Consonant Digraphs

Use **'h'** after consonants to create texture:

| Digraph | Position             | Examples                                                  |
| ------- | -------------------- | --------------------------------------------------------- |
| **kh**  | Any                  | bealakh, klokhan, akhadh, kromhlekh                       |
| **gh**  | Any                  | bhrugh                                                    |
| **th**  | Any                  | rhath, athh, bothn                                        |
| **mh**  | Any                  | uvamh, mhydden, mhund (mh often silent or softens to 'v') |
| **dh**  | Usually medial/final | akhadh, dhall                                             |
| **ph**  | Any                  | rhubha, nhess (not always = 'f' sound)                    |

### 4. The "Rh-" Prefix

A signature element. Use **rh-** at word beginnings for landscape/structural terms:

- rhath, rhyng, rhubha, rhundhaus

### 5. Vowels

- **Prefer short vowels**: a, e, i, o, u
- **Avoid long vowels and modern diphthongs** (no "ee", "oo", "ay")
- **Use diacritics sparingly** for ancient/foreign places: Thuréne, Zulmë, Ymbrë
- **Doubled vowels are rare**: eass is an exception

---

## Naming Conventions by Type

### Male Personal Names

**Short guttural forms**:

- Monosyllabic: Guzz, Prunt, Droz, Welk, Throbb, Globb, Pelch, Grelb
- Pattern: CVC, CCVC, CVCC with hard consonants

**Compound forms**:

- **-vik**: Branvik, Styrovik, Grundvik, Prondvik
- **-brand**: Kolbrand, Golbrand, Pelbrand
- **-bold/-brod**: Helmbrod, Strombold, Thrembold
- **-rik/-rick**: Phodric, Clodrik, Blothric, Moldrick
- **-vax/-vask**: Tholvax, Vask, Korvasch
- **-vast**: Strumvast, Threlvast

**Latinized formal names**:

- Pattern: [Root]-ius, [Root]-us, [Root]-ander
- Examples: Phalantios, Semprovius, Coriandus, Grommius, Thessamander
- Often three names: Semprovius Thessamander Kurz, Grommius Veldassar Bletch

### Female Personal Names

**Elaborate forms**:

- **-andrine/-ine**: Thessalandrine, Threlvandrine, Skellandrine, Phossandrine
- **-enne/-ienne**: Strathenna, Moldravienne, Branvenna
- **-ixia/-axia/-istria**: Kenthrixia, Kolvristria, Valdacrixia, Grondaxia
- **-ella/-lla**: Krombella, Skella, Skrella
- **-ssa/-issa**: Vendrissia, Grundissa, Drelvissa

**Short forms**:

- Monosyllabic: Thel, Stra, Cho, Vru, Stev, Greb
- Diminutive **-ie/-i**: Greffie, Plixie, Heldri, Eldi, Greldi, Sprelli

**Reduplicated names** (distinctive, possibly ritual significance):

- Wiv Wix Via, Kvo Kvo Bren, Kvi Kli Drenn, Vorr Vorr Thess
- Brixabreexa, Tri-Vessa

### Location Names

**Use world-specific terms as prefixes** (from the vocabulary):

| Term   | Meaning      | Example            |
| ------ | ------------ | ------------------ |
| Bhrugh | Stronghold   | Bhrugh Veldriss    |
| Dunn   | Fort         | Dunn Kethram       |
| Kahr   | Royal fort   | Kahr of Throssvel  |
| Benn   | Mountain     | Benn Ithrek        |
| Glenn  | Valley       | Glenn of Vethrim   |
| Strath | Broad valley | Strath Embreth     |
| Korrie | Cirque       | Korrie of Whispers |
| Lhinn  | Pool         | Lhinn of Drothven  |
| Eass   | Waterfall    | Eass Veldrim       |
| Uvamh  | Cave         | Uvamh of Grothen   |
| Vveald | Forest       | Vveald Mossen      |
| Henj   | Stone circle | Henj of Keldros    |

**Settlement suffixes** (Anglo-Saxon/Norse):

- **-wick**: Harrowick, Grothwick
- **-holm/-holme**: Sleckholm, Drabbingholme
- **-worth**: Bremworth, Phlegworth
- **-ford**: Wekkingford
- **-moor/-mire**: Siltmoor, Lowmire, Thrallmire

**Territory suffixes**:

- **-ry/-ery**: Barguestry, Ghastenry, Kelmory
- **-any**: Peltrany, Lurqany, Phossany
- **-mark**: Ghoulmark, Grelmark, Throstlemark

**Fortress suffixes**:

- **-wast**: Skerriwast, Phelmwast
- **-gate**: Churlgate
- **-ling/-ming**: Maudling, Quelming

---

## Anti-Patterns (What to Avoid)

1. **No clean "fantasy" sounds**: Avoid -iel, -wyn, -ara, -iel, -or (Tolkien-esque)
2. **No Japanese/Asian phonology**: Avoid -ko, -shi, -chi endings
3. **No modern English words**: Don't use recognizable words like "Diva"
4. **No bright vowels**: Avoid -ee-, -oo-, -ay- sounds
5. **Minimize 'Z'**: Z-names exist (Zabraq, Zess) but use sparingly
6. **No 'X' as exotic marker**: Except in female -ixia/-axia suffixes
7. **No smooth syllables**: Prefer guttural, stopped, or aspirated sounds

---

## Quick Generation Checklist

When creating a new name/term, verify:

- [ ] Contains at least one consonant cluster OR geminate consonant
- [ ] Uses short vowels (a, e, i, o, u)
- [ ] Has a "weight" to it—nothing too light or musical
- [ ] Could be pronounced by a medieval peasant with missing teeth
- [ ] Evokes stone, damp, cold, or age
- [ ] Doesn't sound like it came from a different fantasy setting
