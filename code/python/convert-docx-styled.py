...
Style-to-Markdown mapping (configurable via STYLE_MAP):
    Heading 2 (140)   -> ## Heading
    DESC HEAD (1_7)    -> ### Sub-heading
    DESCRIPTION (1_4)  -> Regular text
    ACTION (1_3)       -> `Backtick text`
    DIALOGUE (1_5)     -> > Blockquote
    REFLECTION (1_9)   -> > **Bold blockquote**
...
STYLE_MAP = {
    "140":  "heading",       # Heading 2
    "1_7":  "subheading",    # DESC HEAD
    "1_4":  "description",   # DESCRIPTION - regular text
    "1_3":  "action",        # ACTION - backtick text
    "1_5":  "dialogue",      # DIALOGUE - blockquote
    "1_9":  "reflection",    # REFLECTION - bold blockquote
    "1_8":  "list",          # ListStyle (table cells, treated like description)
}
...
def convert_reflection_table(table_element):
    ...
    for para in cell_obj.paragraphs:
        ...
        if not text.strip():
            ...
            role = STYLE_MAP.get(style_id, "reflection")
            if role == "dialogue":
                lines.append(f"> *{text}*")
            else:
                lines.append(f"> **{text}**")
...
def convert_data_table(table_element):
        ...
        for cell_el in cells_el:
            ...
            for para in cell_obj.paragraphs:
                ...
                if ppr is not None:
                    ...
                    if ps is not None:
                        style_id = ps.get(qn("w:val"), "")
                role = STYLE_MAP.get(style_id, "description")
                if role == "subheading":
    ...
    for element in doc.element.body:
        ...
        if tag == "p":
            ...
            if is_reflection_table(element):
                lines = convert_reflection_table(element)
            else:
                lines = convert_data_table(element)
