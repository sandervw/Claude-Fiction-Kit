# [CHARACTER NAME] — Description

_[Work Title] by [Author/Creator]_

## Full Description

1-2 paragraph summary, hard minimum of 250 words, hard maximum of 500 words.

## Description Parts

Each field below should be filled out as "Field_name: Value"

- [symbolic_animal] - a best guess value should always be inferred if not specified in source text
- [gender]
- [rough_age]
- [height]/[build]/[stance]
- [skin_tone]
- [hands]
- [hair] - style/color/condition
- [face] - nose/mouth/brow
- [eyes] - color/shape
- [expression]
- [scars_marks_tattoos] - visible scars, birthmarks, tattoos, body modifications
- [voice]
- [movement] - gait/gestures
- [sound] - caused by movement/gear
- [smell] - breath/general-odor
- [equipment] - list
    - clothing
    - any tools/weapons
    - any headwear
    - any footwear
    - any bags/backpacks
    - any other possessions or personal effects
    - *Name specific materials — wood types, fabric weaves, metal alloys, etc.*

## Sources

- [Source 1 title/description](URL)
- [Source 2 title/description](URL)
- ...